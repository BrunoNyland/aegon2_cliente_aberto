	...
	def __LoadWindow(self):
		...
		try:
			...
			self.pageName = {
				...
				"RANKING"		: "Classificação de Guilds",
				"WAR_LIST"			: "Histórico de Guerras",
			}
			if app.ENABLE_GUILD_SAFEBOX:
...
## GUILD EXP #### GUILD EXP #### GUILD EXP #### GUILD EXP #### GUILD EXP ##
		...
		page.Exp.SetText("Experiência: " + str(curExp) + " de " + str(lastExp))
...
#ENABLE_MSG_LEADER
	...
	def RequestMeeting(self):
		...
		question.SetText("Deseja solicitar Reunião da Guild?")
	...
	def OnPostComment(self):
		...
		if lines[0] == "":
			self.__PopupMessage("Não é permitido palavras com mais de 50 letras.")
			return False
	...
	def OnChangeGradeName(self):
		...
		if gradeName == "Lider" or gradeName == "Líder":
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Nome inválido de patente.")
			return True
	...
	class GuildSafeboxWindow(ui.ScriptWindow):
		...
		def OnMouseOverInMoneySlot(self):
			...
			if rings < 1:
				...
				self.GetChild("tipmoney_text").SetText("Apróx. 1 Máscara da Fortuna")
			else:
				self.GetChild("tipmoney_text").SetText("Apróx. " + localeinfo.NumberToMoneyString(rings) + " Máscaras da Fortuna")
